...
class MatchupSchemeAgent(AnalysisAgent):
    @property
    def name(self) -> str:
        return "matchup_scheme"
        ...
        if cfg.get("is_configured") and provider != "none" and model != "stub-deterministic":
            try:
                ...
                try:
                    ...
                    pass
                llm_result = await call_llm(prompt_template or "You are matchup_scheme for basketball. Return JSON with probabilities HOME/AWAY, confidence, assessment, evidence.", variables, provider, model, base_url, api_key, timeout=12)
                ...
                if not evidence:
                    evidence = [EvidenceItem(feature="llm", observation=llm_result.get("assessment","")[:120], reasoning="Scheme matchup, pace clash")]
                return AgentOutput(
