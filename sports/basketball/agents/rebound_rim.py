...
class ReboundRimAgent(AnalysisAgent):
    @property
    def name(self) -> str:
        return "rebound_rim"
        ...
        if cfg.get("is_configured") and provider != "none" and model != "stub-deterministic":
            try:
                ...
                try:
                    ...
                    pass
                llm_result = await call_llm(prompt_template or "You are rebound_rim for basketball. Return JSON with probabilities HOME/AWAY, confidence, assessment, evidence.", variables, provider, model, base_url, api_key, timeout=12)
                ...
                if not evidence:
                    evidence = [EvidenceItem(feature="llm", observation=llm_result.get("assessment","")[:120], reasoning="Rebound, rim protection, paint")]
                return AgentOutput(
